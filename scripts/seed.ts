...
function run() {
  const seedAll = db.transaction(() => {
    db.exec(
      "DELETE FROM badges; DELETE FROM flashcards; DELETE FROM quiz_questions; DELETE FROM missions;",
    );
  ...
  seedAll();
  console.log(
    `Seeded ${missions.length} missions with quizzes, flashcards, and badges.`,
  );
}
