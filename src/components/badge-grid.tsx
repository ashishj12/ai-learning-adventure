...
export function BadgeGrid({ allBadges, earnedIds }: { allBadges: BadgeT[]; earnedIds: Set<string> }) {
      ...
      {allBadges.map((badge, i) => {
        ...
        return (
          <motion.div
            ...
            transition={{ duration: 0.35, delay: i * 0.05, type: "spring", stiffness: 220, damping: 16 }}
            ...
            <div
              ...
              style={
                earned
                  ...
                  : { boxShadow: "0 0 0 3px var(--paper), 0 0 0 4px rgba(32,29,26,0.12)" }
              }
            >
              {earned ? <Award className="h-7 w-7" aria-hidden /> : <Lock className="h-5 w-5" aria-hidden />}
            </div>
            <p className={cn("mt-2.5 text-sm font-semibold", earned ? "text-ink" : "text-ink/40")}>{badge.name}</p>
