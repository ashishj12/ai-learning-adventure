...
export function MissionMap({ missions }: Props) {
  ...
  return (
    <div className="relative mx-auto max-w-2xl">
      <div aria-hidden className="trail-rule pointer-events-none absolute left-8 top-0 -z-10 h-full w-px sm:left-8" />
        ...
        {missions.map((mission, i) => {
          ...
          const node = (
            ...
            >
              ...
              >
                {isCurrent && (
                  <span className="absolute inset-0 rounded-full border-2 border-amber/50 animate-ping" aria-hidden />
                )}
                {isCompleted ? (
              ...
              <span className="font-stamp text-[10px] font-semibold text-ink/35">WP-{wp}</span>
            </motion.div>
          );
          ...
          const content = (
            <div className="paper-grain min-w-0 flex-1 rounded-2xl border border-ink/10 bg-white p-4 shadow-sm">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                ...
                {isCompleted && <span className="text-xs font-medium text-teal-dark">Completed</span>}
              </div>
              <h3 className="font-display text-lg font-semibold text-ink">{mission.title}</h3>
          ...
          if (isLocked) {
            return (
              <div key={mission.id} aria-disabled title="Complete the previous mission to unlock this one" className="cursor-not-allowed opacity-60">
                {body}
              </div>
            );
          }
          ...
          return (
            <Link key={mission.id} href={`/missions/${mission.slug}`} className="focus-ring rounded-2xl transition-transform hover:-translate-y-0.5">
              {body}
            </Link>
          );
        })}
