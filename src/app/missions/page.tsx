...
export default function MissionsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setError(null);
        const res = await fetch(`/api/missions?sessionId=${encodeURIComponent(sessionId)}`);
        if (!res.ok) throw new Error("Request failed");
        const data = await res.json();
        if (!cancelled) setMissions(data.missions ?? []);
      } catch {
        if (!cancelled) setError("Couldn't load missions right now. Check your connection and try again.");
      }
    }
    load();
    return () => {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="font-display text-3xl font-semibold text-navy">Mission Map</h1>
        <p className="mt-1 text-ink/60">Follow the path — each mission unlocks the next.</p>

        {total > 0 && (
          <div className="mt-5">
            <div className="mb-1 flex justify-between text-xs font-medium text-ink/60">
              <span>{completedCount} of {total} missions complete</span>
              <span>{pct}%</span>
            </div>
      ...
      <div className="mt-10">
        ...
        {!error && missions !== null && missions.length === 0 && (
          <div className="rounded-2xl border border-dashed border-ink/20 p-10 text-center text-ink/60">
            No missions are published yet. Check back soon, or visit the content management area if you're an admin.
          </div>
        )}

        {!error && missions && missions.length > 0 && <MissionMap missions={missions} />}
      </div>
    </div>
  );
}
