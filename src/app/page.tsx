...
import { Compass, ArrowRight, ArrowUpRight, ChevronDown, Send } from "lucide-react";
import { buttonClass } from "@/components/ui/button";
import { useSessionStore } from "@/lib/store/session";

const tickerTerms = [
  "PROMPTS", "LARGE LANGUAGE MODELS", "TOKENS", "HALLUCINATIONS", "BIAS", "RETRIEVAL-AUGMENTED GENERATION", "CONTEXT WINDOWS",
];
...
const faqs = [
  { q: "Do I need to sign in?", a: "No. Your progress is saved automatically in this browser — no account, no email, nothing to set up." },
  { q: "Does the AI tutor cost anything?", a: "The tutor works out of the box using built-in sample responses. If the app owner connects a free-tier AI key, you'll get live answers instead — either way, nothing is required from you." },
  { q: "What if I already know some AI basics?", a: "Missions are grouped by level, from Beginner through Builder Mindset. Jump to whichever mission matches what you already know — the map just asks you to finish one to unlock the next." },
  { q: "Can I revisit a mission later?", a: "Yes — completed missions stay on your map along with your quiz score, and flashcards are always there for review." },
];
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-navy">
        <div className="mx-auto grid max-w-6xl gap-12 px-6 pb-16 pt-16 sm:pt-24 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          ...
          <div>
            ...
            >
              Learn AI, one <em className="font-display italic text-amber">mission</em> at a time.
            </motion.h1>
            ...
            >
              Prompts, LLMs, hallucinations, bias, tokens, RAG — explained through short missions, quizzes,
              flashcards, and a tutor you can actually talk to.
            </motion.p>
            ...
            >
              <label htmlFor="hero-prompt" className="stamp-label mb-2 block text-paper/40">
                Ask the tutor
              </label>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {heroPrompts.map((p) => (
                  <button
                    key={p}
                    type="button"
                    onClick={() => router.push(`/ai-tutor?q=${encodeURIComponent(p)}`)}
            ...
            >
              <Link href="/missions" className={buttonClass({ size: "md" })}>
                {mounted && hasStarted ? "Continue your journey" : "Start your journey"}
              </Link>
              {mounted && hasStarted && lastSlug && (
                <Link href={`/missions/${lastSlug}`} className="text-sm font-medium text-paper/50 underline underline-offset-4 hover:text-paper focus-ring">
                  Jump back in
                </Link>
              )}
            </motion.div>
          ...
          >
            ...
            <div className="absolute inset-6 overflow-hidden rounded-full border border-amber/30">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <Compass className="h-20 w-20 text-paper/90" strokeWidth={1} aria-hidden />
              </div>
              <svg aria-hidden viewBox="0 0 400 400" className="absolute inset-0 h-full w-full opacity-40">
                <circle cx="200" cy="200" r="180" fill="none" stroke="var(--paper)" strokeWidth="1" strokeDasharray="1 8" />
                <circle cx="200" cy="200" r="130" fill="none" stroke="var(--paper)" strokeWidth="1" strokeDasharray="1 8" />
              </svg>
            </div>
          </motion.div>
        </div>
          ...
          {showcase.map((item, i) => (
            ...
            >
              ...
              >
                ...
                <span className="corner-br" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="font-stamp text-6xl font-semibold text-navy/15">{item.n}</span>
                </div>
              </Link>
              ...
              <div className="max-w-md">
                ...
                <h3 className="mt-2 font-display text-3xl font-semibold text-navy sm:text-4xl">
                  {item.title} <em className="italic text-amber-dark">{item.accent}</em>
                </h3>
                ...
                >
                  Open this mission <ArrowRight className="h-4 w-4" aria-hidden />
                </Link>
              </div>
            </motion.div>
          ))}
      ...
      <section className="bg-paper-dim px-6 py-20">
        <div className="mx-auto max-w-3xl">
          ...
          >
            ...
            <h2 className="mt-2 font-display text-3xl font-semibold text-navy">How the journey works</h2>
          </motion.div>

          <ol className="relative">
            <div aria-hidden className="trail-rule absolute bottom-6 left-6 top-6 w-px sm:left-7" />
            {[
              { n: "01", title: "Start a mission", copy: "Pick the next waypoint on the map. Each mission is a short, focused lesson on one AI concept." },
              { n: "02", title: "Ask the tutor", copy: "Stuck on something? Ask for a simpler explanation, another example, or a hint — right inside the mission." },
              { n: "03", title: "Check your answers", copy: "Take the checkpoint quiz. Every answer comes with a plain-language explanation of why it's right or wrong." },
              { n: "04", title: "Earn your badge", copy: "Finish the mission, collect its badge, and get a suggestion for what to explore next." },
            ].map((step, i) => (
              <motion.li
                key={step.n}
      ...
      <section className="bg-paper px-6 py-20">
        <div className="mx-auto max-w-2xl">
          ...
          >
            ...
            <h2 className="mt-2 font-display text-3xl font-semibold text-navy">A few questions</h2>
          </motion.div>
            ...
            {faqs.map((item, i) => {
              ...
              return (
                <div key={item.q} className="border-b border-ink/10">
                  ...
                  >
                    <p className="pb-5 text-sm leading-relaxed text-ink/60">{item.a}</p>
                  </motion.div>
                </div>
              );
            })}
      ...
      <section className="bg-navy px-6 py-24 text-center">
        ...
        >
          ...
          >
            Start your journey
            <ArrowUpRight className="h-8 w-8 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1 sm:h-12 sm:w-12" aria-hidden />
          </Link>
        </motion.div>
      </section>
      ...
      <footer className="border-t border-paper/10 bg-navy px-6 py-6">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-2 text-xs text-paper/35 sm:flex-row">
          <span className="flex items-center gap-1.5">
            <Compass className="h-3.5 w-3.5" aria-hidden /> AI Learning Adventure
          </span>
          <span>A guided journey through AI literacy — no account required.</span>
        </div>
      </footer>
    </div>
  );
