...
const missionSchema = z.object({
  ...
  level: z.enum(["Beginner", "Basic Understanding", "Applied Practice", "Responsible Use", "Builder Mindset"]),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (MissionRepo.getBySlug(parsed.data.slug, false)) {
      return NextResponse.json({ error: "A mission with this slug already exists." }, { status: 409 });
    }
